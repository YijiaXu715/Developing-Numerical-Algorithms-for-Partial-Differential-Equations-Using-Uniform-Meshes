function [bdEdge] = auxstructure_edge(elem)
...
%  where neighbor(t,i) is the global index of the element opposite to the 
...
%  triangle is positive. If not, use elem = fixorder(node,elem) to fix thet
%  order.
%
...
NT = size(elem,1);
elem2edge = uint32(reshape(j,NT,3)); % reshape j into NT(32)*3 matrix shown as elem2edge(32*3)
...
t1 = i1 - NT*(k1-1);
t2 = i2 - NT*(k2-1);
ix = (i1 ~= i2); % ~= i1 is not equal to i2
