function [y,error]=rungekutta_gill(t0,tf,y0,h,ft,fy)
...
for i=1:N
    ...
    k3=ft(t(i)+h./2,y(i)+(sqrt(2)-1)*h*k1./2+(1-sqrt(2)./2)*h*k2);
    k4=ft(t(i)+h,y(i)-(sqrt(2)./2)*h*k2+(1+sqrt(2)./2)*h*k3);
    y(i+1)=y(i)+h*(k1+(2-sqrt(2))*k2+(2+sqrt(2))*k3+k4)./6;
end
yexact= fy(t);
...
% xlabel('t');
% ylabel('y');
% title('Numerical Solution by Runge Kutta Method');
% fig=gcf;
